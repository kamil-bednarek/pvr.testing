...
#include "PVRTestingData.h"

using namespace std;
using namespace ADDON;

PVRTestingData::PVRTestingData(void)
{
  m_iEpgStart = -1;
...
std::string PVRTestingData::GetSettingsFile() const
{
  string settingFile = g_strClientPath;
  if (settingFile.at(settingFile.size() - 1) == '\\' ||
      settingFile.at(settingFile.size() - 1) == '/')
    settingFile.append("PVRTestingAddonSettings.xml");
  else
    settingFile.append("/PVRTestingAddonSettings.xml");
  return settingFile;
}

bool PVRTestingData::LoadDemoData(void)
{
  ...
  if (pElement)
  {
    ...
    while ((pEpgNode = pElement->IterateChildren(pEpgNode)) != NULL)
    {
      ...
      if (!XMLUtils::GetInt(pEpgNode, "channelid", iTmp))
        continue;
      PVRTestingChannel &channel = m_channels.at(iTmp - 1);
...
int PVRTestingData::GetChannelsAmount(void)
{
  return m_channels.size();
}

PVR_ERROR PVRTestingData::GetChannels(ADDON_HANDLE handle, bool bRadio)
{
  for (unsigned int iChannelPtr = 0; iChannelPtr < m_channels.size(); iChannelPtr++)
  {
    PVRTestingChannel &channel = m_channels.at(iChannelPtr);
...
bool PVRTestingData::GetChannel(const PVR_CHANNEL &channel, PVRTestingChannel &myChannel)
{
  for (unsigned int iChannelPtr = 0; iChannelPtr < m_channels.size(); iChannelPtr++)
  {
    PVRTestingChannel &thisChannel = m_channels.at(iChannelPtr);
...
int PVRTestingData::GetChannelGroupsAmount(void)
{
  return m_groups.size();
}

PVR_ERROR PVRTestingData::GetChannelGroups(ADDON_HANDLE handle, bool bRadio)
{
  for (unsigned int iGroupPtr = 0; iGroupPtr < m_groups.size(); iGroupPtr++)
  {
    PVRTestingChannelGroup &group = m_groups.at(iGroupPtr);
...
PVR_ERROR PVRTestingData::GetChannelGroupMembers(ADDON_HANDLE handle, const PVR_CHANNEL_GROUP &group)
{
  for (unsigned int iGroupPtr = 0; iGroupPtr < m_groups.size(); iGroupPtr++)
  {
    PVRTestingChannelGroup &myGroup = m_groups.at(iGroupPtr);
    if (!strcmp(myGroup.strGroupName.c_str(),group.strGroupName))
    {
      for (unsigned int iChannelPtr = 0; iChannelPtr < myGroup.members.size(); iChannelPtr++)
      {
        ...
        if (iId < 0 || iId > (int)m_channels.size() - 1)
          continue;
        PVRTestingChannel &channel = m_channels.at(iId);
...
PVR_ERROR PVRTestingData::GetEPGForChannel(ADDON_HANDLE handle, const PVR_CHANNEL &channel, time_t iStart, time_t iEnd)
{
  if (m_iEpgStart == -1)
    m_iEpgStart = iStart;
  ...
  for (unsigned int iChannelPtr = 0; iChannelPtr < m_channels.size(); iChannelPtr++)
  {
    PVRTestingChannel &myChannel = m_channels.at(iChannelPtr);
    ...
    while (iLastEndTime < iEnd && myChannel.epg.size() > 0)
    {
      ...
      {
        PVRTestingEpgEntry &myTag = myChannel.epg.at(iEntryPtr);
...
int PVRTestingData::GetRecordingsAmount(bool bDeleted)
{
  return bDeleted ? m_recordingsDeleted.size() : m_recordings.size();
}

PVR_ERROR PVRTestingData::GetRecordings(ADDON_HANDLE handle, bool bDeleted)
{
  std::vector<PVRTestingRecording> *recordings = bDeleted ? &m_recordingsDeleted : &m_recordings;

  for (std::vector<PVRTestingRecording>::iterator it = recordings->begin() ; it != recordings->end() ; it++)
  {
    PVRTestingRecording &recording = *it;
...
int PVRTestingData::GetTimersAmount(void)
{
  return m_timers.size();
}

PVR_ERROR PVRTestingData::GetTimers(ADDON_HANDLE handle)
{
  int i = 0;
  for (std::vector<PVRTestingTimer>::iterator it = m_timers.begin() ; it != m_timers.end() ; it++)
  {
    PVRTestingTimer &timer = *it;
